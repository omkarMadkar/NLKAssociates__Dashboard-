...
const INITIAL_WAITING_REPORT = {
  ...
  reportSrNo: "",
  documents: [
    { srNo: 1, name: "Sale Deed / Conveyance Deed", available: "No", remarks: "", fileName: "", filePath: "" },
    { srNo: 2, name: "Index II", available: "No", remarks: "", fileName: "", filePath: "" },
    { srNo: 3, name: "7/12 Extract / Property Card", available: "No", remarks: "", fileName: "", filePath: "" },
    { srNo: 4, name: "Tax Receipt", available: "No", remarks: "", fileName: "", filePath: "" },
    { srNo: 5, name: "Mutation Entry", available: "No", remarks: "", fileName: "", filePath: "" },
  ],
};
...
export default function TSRInitiation() {
  ...
  const handleAddDoc = () => {
    setWaitingReport(prev => ({
      ...prev,
      documents: [...prev.documents, { srNo: prev.documents.length + 1, name: "", available: "No", remarks: "", fileName: "", filePath: "" }]
    }));
  };
  ...
  const handleViewFile = (filePath, originalName = "") => {
    ...
    const url = cleanPath.startsWith("http") ? cleanPath : `${BACKEND_URL}/${cleanPath}`;
          ...
          if (data.fileInfo) {
            newForm.uploadedDocuments = [
              ...(prev.uploadedDocuments || []),
              {
                ...
                extractedFields: Object.keys(data.extractedFields).filter((k) => data.extractedFields[k]),
                uploadedAt: new Date(),
              },
            ];
          }
        ...
        <form onSubmit={handleSubmit} style={{ padding: 28 }}>
          ...
          >
            ...
            <div style={{ marginLeft: "auto" }}>
              ...
              {(activeTab === "otherProvisions" || activeTab === "waitingReport") && (
                <button
                ...
                >
                  {activeTab === "waitingReport" ? (submitting ? "Initiating..." : "Initiate TSR Report ✓") : "Next Section ➔"}
                </button>
              )}
            </div>
          </div>
      ...
      {viewingRecord && createPortal(
        ...
        >
          ...
          >
            ...
            >
              <div>
                <h2 style={{ margin: 0, fontSize: 20, fontFamily: "Playfair Display" }}>
                  TSR Initiation Details — {viewingRecord.appId}
                </h2>
                <span style={{ fontSize: 12, opacity: 0.8 }}>Created on {new Date(viewingRecord.createdAt).toLocaleDateString("en-IN")}</span>
              </div>
              <button
            ...
            <div style={{ padding: 28, overflowY: "auto", display: "flex", flexDirection: "column", gap: 32 }}>
              
              {/* Section 1: Basic Info */}
              <div>
                <h3 style={{ borderBottom: "2px solid #f1f5f9", paddingBottom: 8, color: "var(--navy)", margin: "0 0 16px 0", fontSize: 15, fontWeight: 700 }}>
                  I. Basic Info & Particulars
                </h3>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px 24px", fontSize: 13, marginBottom: 16 }}>
                  <div><strong>Author/Advocate:</strong> <span style={{ color: "#475569" }}>{viewingRecord.author || "—"}</span></div>
                  <div><strong>TSR Reference No:</strong> <span style={{ color: "#475569" }}>{viewingRecord.refNo || "—"}</span></div>
                  <div><strong>Branch:</strong> <span style={{ color: "#475569" }}>{viewingRecord.branch || "—"}</span></div>
                  <div><strong>Initiation Date:</strong> <span style={{ color: "#475569" }}>{viewingRecord.initiationDate || "—"}</span></div>
                  <div><strong>Applicant:</strong> <span style={{ color: "#475569" }}>{viewingRecord.applicant || "—"}</span></div>
                  <div><strong>Co-Applicant:</strong> <span style={{ color: "#475569" }}>{viewingRecord.coApplicant || "—"}</span></div>
                  <div><strong>Existing Owner:</strong> <span style={{ color: "#475569" }}>{viewingRecord.existingOwner || "—"}</span></div>
                  <div><strong>Transaction Type:</strong> <span style={{ color: "#475569" }}>{viewingRecord.transactionType || "—"}</span></div>
                  <div><strong>Bank Branch:</strong> <span style={{ color: "#475569" }}>{viewingRecord.bankBranch || "—"}</span></div>
                </div>
                ...
                {viewingRecord.uploadedDocuments?.length > 0 && (
                  <div style={{ background: "#f8fafc", padding: 16, borderRadius: 8, border: "1px solid #e2e8f0", marginTop: 16 }}>
                    <h4 style={{ margin: "0 0 12px 0", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: 0.5 }}>
                      OCR Scanned / Primary Documents
                    </h4>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {viewingRecord.uploadedDocuments.map((doc, idx) => (
                        <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "white", padding: "8px 12px", borderRadius: 6, border: "1px solid #e2e8f0", fontSize: 12 }}>
                          <div>
                            <span style={{ fontWeight: 600, color: "var(--black)" }}>{doc.originalName}</span>
                            <span style={{ color: "#64748b", marginLeft: 8 }}>({(doc.fileSize / 1024).toFixed(1)} KB)</span>
                            {doc.extractedFields?.length > 0 && (
                              <div style={{ fontSize: 10, color: "#059669", marginTop: 2 }}>
                                Extracted fields: {doc.extractedFields.join(", ")}
                              </div>
                            )}
                          </div>
                          <button
                            type="button"
                            onClick={() => handleViewFile(doc.filePath, doc.originalName)}
                            style={{
                              background: "#eff6ff",
                              color: "#1d4ed8",
              ...
              <div>
                <h3 style={{ borderBottom: "2px solid #f1f5f9", paddingBottom: 8, color: "var(--navy)", margin: "0 0 16px 0", fontSize: 15, fontWeight: 700 }}>
                  II. Property Details & Boundaries
                </h3>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px 24px", fontSize: 13, marginBottom: 16 }}>
                  <div><strong>Municipal Property No:</strong> <span style={{ color: "#475569" }}>{viewingRecord.municipalPropertyNo || "—"}</span></div>
                  <div><strong>RCC Construction Area:</strong> <span style={{ color: "#475569" }}>{viewingRecord.rccConstructionArea || "—"}</span></div>
                  <div><strong>Village:</strong> <span style={{ color: "#475569" }}>{viewingRecord.village || "—"}</span></div>
                  <div><strong>Taluka:</strong> <span style={{ color: "#475569" }}>{viewingRecord.taluka || "—"}</span></div>
                  <div><strong>District:</strong> <span style={{ color: "#475569" }}>{viewingRecord.district || "—"}</span></div>
                  <div><strong>Municipal Council:</strong> <span style={{ color: "#475569" }}>{viewingRecord.municipalCouncil || "—"}</span></div>
                </div>
                <div style={{ background: "#f8fafc", padding: 16, borderRadius: 8, border: "1px solid #e2e8f0" }}>
                  <h4 style={{ margin: "0 0 12px 0", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: 0.5 }}>Boundaries</h4>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 16, fontSize: 13 }}>
                    <div><strong>East:</strong> <span style={{ color: "#475569" }}>{viewingRecord.boundaryEast || "—"}</span></div>
                    <div><strong>West:</strong> <span style={{ color: "#475569" }}>{viewingRecord.boundaryWest || "—"}</span></div>
                    <div><strong>South:</strong> <span style={{ color: "#475569" }}>{viewingRecord.boundarySouth || "—"}</span></div>
                    <div><strong>North:</strong> <span style={{ color: "#475569" }}>{viewingRecord.boundaryNorth || "—"}</span></div>
                  </div>
                </div>
              </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden" }}>
                    ...
                    <tbody>
                      {viewingRecord.landParcels.map((p, idx) => (
                        <tr key={idx} style={{ borderBottom: idx < viewingRecord.landParcels.length - 1 ? "1px solid #f1f5f9" : "none" }}>
              ...
              {viewingRecord.otherProvisionId?.answers?.length > 0 && (
                <div>
                  <h3 style={{ borderBottom: "2px solid #f1f5f9", paddingBottom: 8, color: "var(--navy)", margin: "0 0 16px 0", fontSize: 15, fontWeight: 700 }}>
                    IV. Other Provisions Scrutiny
                  </h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 250, overflowY: "auto", paddingRight: 8, border: "1px solid #e2e8f0", borderRadius: 8, padding: 16 }}>
                    {viewingRecord.otherProvisionId.answers.map((ans, idx) => (
                      <div key={idx} style={{ borderBottom: idx < viewingRecord.otherProvisionId.answers.length - 1 ? "1px solid #f1f5f9" : "none", paddingBottom: 8, marginBottom: 8, fontSize: 13 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 600 }}>
                          <span>{ans.code}. {ans.question}</span>
                          <span style={{ 
                            color: ans.answer === "Yes" ? "#047857" : ans.answer === "No" ? "#b91c1c" : "#475569",
                            background: ans.answer === "Yes" ? "#d1fae5" : ans.answer === "No" ? "#fee2e2" : "#f1f5f9",
                            padding: "2px 8px",
                            borderRadius: 4,
                            fontSize: 11
                          }}>
              ...
              {/* Section 5: Waiting Report & Checklist */}
              {viewingRecord.waitingReportId && (
                <div>
                  <h3 style={{ borderBottom: "2px solid #f1f5f9", paddingBottom: 8, color: "var(--navy)", margin: "0 0 16px 0", fontSize: 15, fontWeight: 700 }}>
                    V. Waiting Report & Pending Documents
                  </h3>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, fontSize: 13, marginBottom: 16 }}>
                    <div><strong>Chalan No:</strong> <span style={{ color: "#475569" }}>{viewingRecord.waitingReportId.chalanNo || "—"}</span></div>
                    <div><strong>Chalan Date:</strong> <span style={{ color: "#475569" }}>{viewingRecord.waitingReportId.date || "—"}</span></div>
                    <div><strong>Report Sr No:</strong> <span style={{ color: "#475569" }}>{viewingRecord.waitingReportId.reportSrNo || "—"}</span></div>
                  </div>
                  ...
                  {viewingRecord.waitingReportId.documents?.length > 0 && (
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden" }}>
                      <thead>
                        <tr style={{ background: "#f8fafc" }}>
                          <th style={{ padding: "8px 12px", borderBottom: "1px solid #e2e8f0", width: 40, textAlign: "center" }}>Sr.</th>
                          <th style={{ padding: "8px 12px", borderBottom: "1px solid #e2e8f0", textAlign: "left" }}>Document Name</th>
                          <th style={{ padding: "8px 12px", borderBottom: "1px solid #e2e8f0", width: 80, textAlign: "center" }}>Available</th>
                          <th style={{ padding: "8px 12px", borderBottom: "1px solid #e2e8f0", textAlign: "left" }}>Remarks</th>
                          <th style={{ padding: "8px 12px", borderBottom: "1px solid #e2e8f0", width: 90, textAlign: "center" }}>Attachment</th>
                        </tr>
                      </thead>
                      <tbody>
                        {viewingRecord.waitingReportId.documents.map((d, idx) => (
                          <tr key={idx} style={{ borderBottom: idx < viewingRecord.waitingReportId.documents.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                            <td style={{ padding: "8px 12px", textAlign: "center" }}>{d.srNo}</td>
                            <td style={{ padding: "8px 12px", fontWeight: 500 }}>{d.name}</td>
                            <td style={{ padding: "8px 12px", textAlign: "center" }}>
                              <span style={{
                                color: d.available === "Yes" ? "#047857" : "#b91c1c",
                                fontWeight: 600
                              }}>
                                {d.available}
                              </span>
                            </td>
                            <td style={{ padding: "8px 12px" }}>{d.remarks || "—"}</td>
                            <td style={{ padding: "8px 12px", textAlign: "center" }}>
                              {d.filePath ? (
                                <button
                                  type="button"
                                  onClick={() => handleViewFile(d.filePath, d.name)}
                                  style={{
                                    background: "#eff6ff",
                                    color: "#1d4ed8",
                                    border: "1px solid #bfdbfe",
              ...
              {viewingRecord.titleFlowId && (
                <div>
                  <h3 style={{ borderBottom: "2px solid #f1f5f9", paddingBottom: 8, color: "var(--navy)", margin: "0 0 16px 0", fontSize: 15, fontWeight: 700 }}>
                    VI. Title Flow Events ({viewingRecord.titleFlowId.events?.length || 0} Events)
                  </h3>
                  {viewingRecord.titleFlowId.events?.length > 0 ? (
                    <div style={{ maxHeight: 350, overflowY: "auto", border: "1px solid #e2e8f0", borderRadius: 8 }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, textAlign: "left" }}>
                        ...
                        <tbody>
                          {viewingRecord.titleFlowId.events.map((evt, idx) => (
                            <tr key={idx} style={{ borderBottom: "1px solid #f1f5f9", background: evt.currentOwner === "YES" ? "#f0fdf4" : "transparent" }}>
                              ...
                              <td style={{ padding: "10px 12px" }}>
                                ...
                                {evt.currentOwner === "YES" && (
                                  <span style={{ color: "#16a34a", fontSize: 10, fontWeight: 700, marginLeft: 6 }}>
                                    (Owner)
                                  </span>
                                )}
                              </td>
                              <td style={{ padding: "10px 12px" }}>{evt.documentType || "—"}</td>
                              <td style={{ padding: "10px 12px" }}>{evt.registrationNo ? `${evt.registrationNo} (${evt.sroName || ""})` : "—"}</td>
          ...
          >
            ...
            <div style={{ flex: 1, position: "relative", background: "#f8fafc", overflow: "hidden" }}>
              {previewFile.isPdf ? (
                ...
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%", overflow: "auto", padding: 20 }}>
                  <img
                    src={previewFile.url}
                    alt={previewFile.name}
                    style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain", borderRadius: 8, boxShadow: "0 4px 6px -1px rgba(0,0,0,0.1)" }}
                  />
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", height: "100%", gap: 16, padding: 40, textAlign: "center" }}>
